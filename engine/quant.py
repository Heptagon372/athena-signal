...
def ou_half_life(prices, window: int = 60) -> dict | None:
    ...
    ⚠ **window 기본값 60 에 주의하세요.** 이 함수는 넘겨받은 시계열이 아무리
      길어도 **마지막 `window` 개만** 씁니다. 60개 표본에서 AR(1) 계수는
      Kendall 소표본 편향으로 크게 과소추정되고, 반감기가 절반 이하로 나옵니다.
      이 저장소 실측 (OU 인공데이터 30회 평균):

          참 반감기  4.6일 -> 창60: 2.8일   창 전체: 4.3일
          참 반감기  6.9일 -> 창60: 5.4일   창 전체: 6.9일
          참 반감기 13.9일 -> 창60: 9.1일   창 전체: 13.9일

      **반감기 자체가 판단 근거일 때는 `window=len(series)` 를 명시하세요.**
      국면 판별(indicators.detect_regime)처럼 "최근 상태"를 보는 용도라면
      기본값 60 이 의도된 동작입니다. engine/pairs.py 는 전체 창을 씁니다.
    """
    p = np.asarray(prices, dtype=float)
    p = p[np.isfinite(p)]
    if len(p) < window:
